...
/*
 * Changed By : Rushika Patel
 * Date : 28-OCT-2015
 * Change : Callback featire changes
 * */
if (isset($_GET["selectagent_new"]))			{$selectagent_new=$_GET["selectagent_new"];}
	elseif (isset($_POST["selectagent_new"]))	{$selectagent_new=$_POST["selectagent_new"];}
/*****************************************************************************/
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ($show_campaign_list > 0)
		...
		if ($show_language_list > 0)
			...
			echo "</br><select class='form-control' size=1 name=VD_language id=VD_language>\n";
...
if ($ACTION == 'updateDISPO')
	...
	if ( ($dispo_choice == 'CBHOLD') and (strlen($CallBackDatETimE)>10) )
		...
		$comments = preg_replace('/;/i','',$comments);
		$comments = preg_replace("/\\\\/i",' ',$comments);
		$stmt="INSERT INTO vicidial_callbacks (lead_id,list_id,campaign_id,status,entry_time,callback_time,user,recipient,comments,user_group,lead_status) values('$lead_id','$list_id','$campaign','ACTIVE','$NOW_TIME','$CallBackDatETimE','$selectagent_new','$recipient','$comments','$user_group','$CallBackLeadStatus');";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_to_mysqli($stmt, $link);
...
if ($ACTION == 'getAgentList')
	...
	$stmt="SELECT user from vicidial_users where user_group='$VU_user_group' and user!='$user'";
	$rslt=mysql_to_mysqli($stmt, $link);
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00574',$VD_login,$server_ip,$session_name,$one_mysql_log);}
	$row=mysqli_fetch_assoc($rslt);
	$dropdown_value="<select name='new_agent' id='new_agent' class='form-control'><option value='$user'>$user</option>";
		while ($val = mysqli_fetch_assoc($rslt))
		{
				$dropdown_value.="<option value='".$val['user']."'>".$val['user']."</option>";
		}
		echo $dropdown_value."</select>";
	}
/*************************************************************/
...
if ($ACTION == 'AGENTSview')
	...
	while ($agents_count > $loop_count)
		...
		if ( ($status=='READY') or ($status=='CLOSER') ) 
			{
			$statuscolor='#ADD8E6';
			$statusclass='col-md-12 padding5 enjoy-css_cust1';
			$call_time = ($StarTtime - $call_finish);
			}
		if ( ($status=='QUEUE') or ($status=='INCALL') ) 
			{
			$statuscolor='#D8BFD8';
			$statusclass='col-md-12 padding5 enjoy-css_red1';
			$call_time = ($StarTtime - $call_start);
			}
		if ($status=='PAUSED') 
			{
			$statuscolor='#F0E68C';
			$statusclass='col-md-12 padding5 enjoy-css_yellow1';
			$call_time = ($StarTtime - $call_finish);
			}
	...
	if ($comments=='AgentXferViewSelect') 
		{
		echo "<TABLE CELLPADDING=0 CELLSPACING=1 width='50%'><TR><TD VALIGN=TOP><TABLE CELLPADDING=0 CELLSPACING=1 width='100%'>";
...
if ($ACTION == 'CALLSINQUEUEview')
	...
	if (preg_match('/NONE/i',$view_calls_in_queue))
		...
		echo "<TABLE class=\"hover\" CELLPADDING=0 CELLSPACING=1 BORDER=0 WIDTH='100%'>";
		echo "<thead>";
		echo "<TR>";
		echo "<th> &nbsp; </th>";
		echo "<th><B> &nbsp; "._QXZ("PHONE")." &nbsp;</b> </th>";
		echo "<th><B> &nbsp; "._QXZ("FULL NAME")." &nbsp;</b> </th>";
		echo "<th><B> &nbsp; "._QXZ("WAIT")." &nbsp; </b></th>";
		echo "<th><B> &nbsp; "._QXZ("AGENT")." &nbsp; </b></th>";
		echo "<th> &nbsp; &nbsp; &nbsp; </b></th>";
		echo "<th><B> &nbsp; "._QXZ("CALL GROUP")." &nbsp; </b></th>";
		echo "<th><B> &nbsp; "._QXZ("TYPE")." &nbsp; </b></th>";
		echo "</TR></thead></table>";
...
if ($ACTION == 'CALLLOGview')
	...
	echo "<input class=\"form-control\" type=text name=calllogdate id=calllogdate value=\"$date\" size=12 maxlength=10> </br>";
	echo "<div class=\"foot\">";
	echo "<button class=\"btn btn-default\" type=\"button\" onclick=\"VieWCalLLoG('','form');return false;\">"._QXZ("GO")."</button> &nbsp;  &nbsp; &nbsp; ";
	echo "<button class=\"btn btn-primary\" type=\"button\" onclick=\"hideDiv('CalLLoGDisplaYBox');return false;\"> "._QXZ("close")." </button>";
	echo "</B></font>\n";
	echo "<BR>\n";
	echo "<BR>\n";
	...
	while ($g > $u) 
		...
		echo "<td> $ALLcampaign_id[$i] </td>\n";
		echo "<td>$ALLin_out[$i] </td>\n";
		echo "<td>$ALLalt_dial[$i] </td>\n";
		echo "<td>$ALLhangup_reason[$i] </td>\n";
		echo "<td><a href=\"#\" onclick=\"VieWLeaDInfO($ALLlead_id[$i]);return false;\"> "._QXZ("INFO")."</A> </td>\n";
...
echo "</tbody>";
	echo "</TABLE>";
	echo "<BR>";
	echo "<a class=\"btn btn-primary\" href=\"#\" onclick=\"CalLLoGVieWClose();return false;\">"._QXZ("Close Call Log")."</a>";
	echo "</CENTER>";
	}
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		if ($counts_to_print > 0) 
			...
			echo "</B></font>\n";
			echo "<BR>\n";
			echo "<TABLE class=\"hover\" bgcolor=#CCFFCC CELLPADDING=0 CELLSPACING=1 BORDER=0 WIDTH=100%>";
			echo "<THEAD>";
			echo "<TR>";
			echo "<TH>#</TH>";
			echo "<TH>"._QXZ("FULL NAME")."</TH>";
			...
			if ($search_result_count)
				...
				if ($g < 1)
					{echo "<tr><td colspan=10 align=center>"._QXZ("No results found")."</td></tr>";}
				...
				while ($g > $u) 
					...
					echo "<td> $ALLphone_code[$i] $ALLphone_number[$i] </td>\n";
					echo "<td> $ALLstatus[$i] </td>\n";
					echo "<td> $ALLcall_date[$i] </td>\n";
					echo "<td> $ALLcity[$i] </td>\n";
					echo "<td> $ALLstate[$i]</td>\n";
					echo "<td> $ALLpostal_code[$i] </td>\n";
					echo "<td> <a href=\"#\" onclick=\"VieWLeaDInfO($ALLlead_id[$i],'','$inbound_lead_search');return false;\"> "._QXZ("INFO")." </A> </td>\n";
			...
			echo "<BR>";
			echo "<div class=\"foot\">\n";
			echo "<a class=\"btn btn-primary\" href=\"#\" onclick=\"hideDiv('SearcHResultSDisplaYBox');return false;\">"._QXZ("Go Back")."</a>";
			echo "</div>";
			echo "</CENTER>";
			}
		else
...
if ($ACTION == 'SEARCHCONTACTSRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		elseif (strlen($location) > 0)
			...
			echo "<BR><BR>";
			echo "<a class='btn btn-primary' href=\"#\" onclick=\"hideDiv('SearcHResultSContactsBox');return false;\">"._QXZ("Go Back")."</a>";
			echo "</CENTER>";
		...
		if ($counts_to_print > 0)
			...
			echo "<thead><TR>";
			echo "<TH > # </font></TD>";
			echo "<TH >"._QXZ("FIRST NAME")." </TD>";
			echo "<TH >"._QXZ("LAST NAME")." </TD>";
			echo "<TH >"._QXZ("OFFICE")." </TD>";
			echo "<TH >"._QXZ("MOBILE")." </TD>";
			echo "<TH >"._QXZ("OTHER 1")." </TD>";
			echo "<TH >"._QXZ("OTHER 2")." </TD>";
			echo "<TH >"._QXZ("BU NAME")." </TD>";
			echo "</TR>";
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if (strlen($callback_id) > 0)
			...
			if ($cb_to_print > 0)
				{
				$row=mysqli_fetch_row($rslt);
				echo "<TABLE CELLPADDING=0 CELLSPACING=1 BORDER=0 WIDTH=770>";
		...
		$INFOout .= "<TABLE class=\"col_table table pull-left\" CELLPADDING=0 CELLSPACING=1 BORDER=0 WIDTH=100%>";
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\"col-md-6\">"._QXZ("Status:")." </div> <div class=\"col-md-6\">$row[0]</div></span></td>";
			if ( ($label_vendor_lead_code!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_vendor_lead_code:</div>  <div class=\"col-md-6 \">$row[1]</div></span></td></tr>";}
			$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">"._QXZ("List ID:")."</div> <div class=\" col-md-6\">$row[2]</div></span></td>";
			$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">"._QXZ("Timezone:")." </div><div class=\" col-md-6\">$row[3]</div></span></td></tr>";

			$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">"._QXZ("Called Since Last Reset:")."</div><div class=\"col-md-6\">$row[4]</div></span></td>";
			if ( ($label_phone_code!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_phone_code: </div><div class=\" col-md-6\">$row[5]</div></td></tr>";}
			if ( ($label_phone_number!='---HIDE---') or ($label_hide_field_logs=='N') )
				{
				$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_phone_number: </div><div class=\"col-md-6\"> - &nbsp; &nbsp; &nbsp; &nbsp; ";
				...
				$INFOout .= "</div></span></td></tr>";
				}
			if ( ($label_phone_number=='---HIDE---') and ($hide_dial_links < 1) )
				{
				if ($manual_dial_filter > 0)
					{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">"._QXZ("Dial Link:")." </div><div class=\" col-md-6\"><a href=\"#\" onclick=\"NeWManuaLDiaLCalL('CALLLOG',$row[5], $row[6], $lead_id);return false;\"> "._QXZ("DIAL")." </a></div></span></td>";}
				else
					{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">"._QXZ("Dial Link:")."</div><div class=\" col-md-6\">"._QXZ(" DIAL ");}
				}
			$INFOout .= "</div></span></td>";
			if ($inbound_lead_search > 0)
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\"><a href=\"#\" onclick=\"LeaDSearcHSelecT('$lead_id');return false;\">"._QXZ("SELECT THIS LEAD")."</a></div><div class=\" col-md-6\">&nbsp;</div></span></td></tr>";}
			if ( ($label_title!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_title: </div><div class=\" col-md-6\">$row[7]</div></span></td>";}
			if ( ($label_first_name!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_first_name: </div><div class=\" col-md-6\">$row[8]</div></span></td></tr>";}
			if ( ($label_middle_initial!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_middle_initial: </div><div class=\" col-md-6\">$row[9]</div></span></td>";}
			if ( ($label_last_name!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_last_name: </div><div class=\" col-md-6\">$row[10]</div></span></td></tr>";}
			if ( ($label_address1!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_address1:</div><div class=\" col-md-6\">$row[11]</div></span></td>";}
			if ( ($label_address2!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_address2: </div><div class=\" col-md-6\">$row[12]</div></span></td></tr>";}
			if ( ($label_address3!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_address3: </div><div class=\" col-md-6\">$row[13]</div></span></td>";}
			if ( ($label_city!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_city: </div><div class=\" col-md-6\">$row[14]</div></span></td></tr>";}
			if ( ($label_state!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_state: </div><div class=\" col-md-6\">$row[15]</div></span></td>";}
			if ( ($label_province!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_province: </div><div class=\" col-md-6\">$row[16]</div></span></td></tr>";}
			if ( ($label_postal_code!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_postal_code:</div><div class=\" col-md-6\">$row[17]</div></span></td>";}
			$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">Country: </div><div class=\" col-md-6\">$row[18]</div></span></td></tr>";
			if ( ($label_gender!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<tr ><tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_gender:</div><div class=\" col-md-6\">$row[19]</div></span></td>";}
			if ( ($label_alt_phone!='---HIDE---') or ($label_hide_field_logs=='N') )
				{
				$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_alt_phone: </div><div class=\" col-md-6\"> - &nbsp; &nbsp; &nbsp; &nbsp; ";
				if ($hide_dial_links < 1)
					{
					if ($manual_dial_filter > 0)
			...
			if ( ($label_email!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_email: </div><div class=\" col-md-6\">$row[21]</div></span></td>";}
			if ( ($label_security_phrase!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_security_phrase: </div><div class=\" col-md-6\">$row[22]</div></span></td></tr>";}
			if ( ($label_comments!='---HIDE---') or ($label_hide_field_logs=='N') )
				{$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$label_comments: </div><div class=\" col-md-6\">$row[23]</div></span></td>";}
			if ($hide_call_log_info=='N')
				{$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">"._QXZ("Called Count:")." </div><div class=\" col-md-6\">$row[24]</div></span></td></tr>";}
			$INFOout .= "<tr ><td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">"._QXZ("Last Local Call Time:")." </div><div class=\" col-md-6\">$row[25]</div></span></td>";
	...
	#		$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Entry List ID: &nbsp; </td><td ALIGN=left><font size=2>$row[28]</td></tr>";
					...
					if (strlen($select_SQL) > 0)
						...
						if ($list_lead_ct > 0)
							...
							while ($fields_to_print > $o) 
								...
								$INFOout .= "<td class=\"col-md-6  \"><span class=\"col_border col-md-12 no-padding\"><div class=\" col-md-6\">$A_field_name[$o]: </div><div class=\" col-md-6\">$A_field_value</div></span></td></tr>";
		...
		#	$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\" COLSPAN=9><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; FULL NAME &nbsp; </font></TD>";
			$NOTESout .= "</TR>";*/
			$NOTESout .= "<CENTER><div class=\"head_title col-md-12\" style=\"width:100%;\">"._QXZ("CALL LOG FOR THIS LEAD:")."</div>\n";}
			$NOTESout .= "<TABLE class=\"hover\" CELLPADDING=0 CELLSPACING=1 BORDER=0 >";
			$NOTESout .= "<THEAD>";
			$NOTESout .= "<TR>";
			$NOTESout .= "<TH>#</TH>";
		...
		#	$NOTESout .= "<TD BGCOLOR=\"#CCCCCC\" COLSPAN=9><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; FULL NAME &nbsp; </font></TD>";
			...
			while ($g > $u) 
				...
				$NOTESout .= "<td align=right><font size=2> $ALLphone_code[$i]  </td>\n";
		...
		if ($allow_emails>0)
			...
			$NOTESout .= "<TABLE class='hover' CELLPADDING=0 CELLSPACING=1 BORDER=0 width='98%'>";
			$NOTESout .= "<thead>";
			$NOTESout .= "<TR>";
			$NOTESout .= "<th><B> # </B></th>";
			$NOTESout .= "<th><B> &nbsp; "._QXZ("DATE/TIME")." </B></th>";
			$NOTESout .= "<th><B> &nbsp; "._QXZ("AGENT")." </B></th>";
			$NOTESout .= "<th><B> &nbsp; "._QXZ("CAMPAIGN")." </B></th>";
			$NOTESout .= "<th><B> &nbsp; "._QXZ("EMAIL TO")." </B></th>";
			$NOTESout .= "<th><B> &nbsp; "._QXZ("ATTACHMENTS")." </B></th>";
			$NOTESout .= "</tr>\n";
			$NOTESout .= "</thead>";
