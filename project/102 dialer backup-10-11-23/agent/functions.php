...
function custom_list_fields_values($lead_id,$list_id,$uniqueid,$user)
	...
	if ($tablecount_to_print > 0) 
		...
		if ($fieldscount_to_print > 0) 
			...
			$CFoutput .= "<input type=hidden name=stage id=stage class='btn btn-bdr-sucess' value=\"SUBMIT\">\n";
			...
			while ($fields_to_print > $o) 
				...
				if ($A_field_type[$o]=='SELECT')
					{
					$field_HTML .= "<select class='form-control col-sm-9' size=1 name=$A_field_label[$o] id=$A_field_label[$o]>\n";
					}
				if ($A_field_type[$o]=='MULTI')
					{
					$field_HTML .= "<select class='form-control col-sm-9' MULTIPLE size=$A_field_size[$o] name=$A_field_label[$o][] id=$A_field_label[$o][]>\n";
					}
					...
					while ($te < $field_options_count)
						...
						if (preg_match("/,/",$field_options_array[$te]))
							...
							if ( ($A_field_type[$o]=='RADIO') or ($A_field_type[$o]=='CHECKBOX') )
								...
								$field_HTML .= "<input class='ace' type=$A_field_type[$o] name=$A_field_label[$o][] id=$A_field_label[$o][] value=\"$field_options_value_array[0]\" $field_selected> <lable class='lbl'>"._QXZ("$field_options_value_array[1]")."</lable>\n";
								if ($A_multi_position[$o]=='VERTICAL') 
									{$field_HTML .= "<BR>\n";}
								}
							}
				...
				if ($A_field_type[$o]=='TEXT') 
					...
					$field_HTML .= "<input class='form-control col-sm-9' type=text size=$A_field_size[$o] maxlength=$A_field_max[$o] name=$A_field_label[$o] id=$A_field_label[$o] value=\""._QXZ("$A_field_value[$o]")."\">\n";
					}
				if ($A_field_type[$o]=='AREA') 
					{
					if ($A_field_default[$o]=='NULL') {$A_field_default[$o]='';}
					if (strlen($A_field_value[$o]) < 1) {$A_field_value[$o] = $A_field_default[$o];}
					$field_HTML .= "<textarea class='form-control col-sm-9' name=$A_field_label[$o] id=$A_field_label[$o] ROWS=$A_field_max[$o] COLS=$A_field_size[$o]>$A_field_value[$o]</textarea>";
					}
				...
				if ($A_field_type[$o]=='READONLY')
					...
					if ($A_field_default[$o]=='NULL') {$A_field_default[$o]='';}
				#	if (strlen($A_field_value[$o]) < 1) {$A_field_value[$o] = $A_field_default[$o];}
					$field_HTML .= "<input class='form-control col-sm-9' type=hidden name=$A_field_label[$o] id=$A_field_label[$o] value=\"$A_field_value[$o]\"> "._QXZ("$A_field_value[$o]")."\n";
					}
				if ( ($A_field_type[$o]=='HIDDEN') or ($A_field_type[$o]=='HIDEBLOB') )
					{
					if (strlen($A_field_value[$o]) < 1) {$A_field_value[$o] = $A_field_default[$o];}
					if ($A_field_default[$o]=='NULL') {$A_field_default[$o]='';}
					$field_HTML .= "<input class='form-control col-sm-9' type=hidden name=$A_field_label[$o] id=$A_field_label[$o] value=\"$A_field_value[$o]\">\n";
					}
				...
				if ($A_field_type[$o]=='DATE') 
					...
					$field_HTML .= "<input class='form-control col-sm-4' type=text size=11 maxlength=10 name=$A_field_label[$o] id=$A_field_label[$o] value=\"$default_date\" onclick=\"f_tcalToggle()\">\n";
				...
				if ($A_field_type[$o]=='TIME') 
					...
					$field_HTML .= "<input class='form-control col-sm-4' type=hidden name=$A_field_label[$o] id=$A_field_label[$o] value=\"$default_time\">";
					$field_HTML .= "<SELECT class='form-control col-sm-4' name=HOUR_$A_field_label[$o] id=HOUR_$A_field_label[$o]>";
