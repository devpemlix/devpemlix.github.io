...
if ( (!$vendor_id) and (!$phone)  and (!$lead_id) and (!$log_phone)  and (!$log_lead_id) and (!$log_phone_archive)  and (!$log_lead_id_archive) and ( (strlen($status)<1) and (strlen($list_id)<1) and (strlen($user)<1) and (strlen($owner)<1) ) and ( (strlen($first_name)<1) and (strlen($last_name)<1) and (strlen($email)<1) ))
	...
	if (mysqli_num_rows($archive_rslt)>0) 
		{
		echo "<TR >";
		echo "<TD>"._QXZ("Archive search").": &nbsp; </TD><TD ALIGN=left><div class=\"col-lg-5\"><select class=\"form-control\" size=1 name=archive_search><option>"._QXZ("No")."</option><option>"._QXZ("Yes")."</option><option	SELECTED>$archive_search</option></select></div></TD>";
		echo "<TD> &nbsp; </TD>\n";
		echo "</TR><TR >";
	...
	echo "</TR><TR >";
	echo "<TD class=\"col-md-3\">$label_alt_phone "._QXZ("search").": &nbsp; </TD><TD class=\"col-md-3\" ALIGN=left><div class=\"col-lg-12\"><select class=\"form-control\" size=1 name=alt_phone_search><option>"._QXZ("No")."</option><option>"._QXZ("Yes")."</option><option SELECTED>$alt_phone_search</option></select></div></TD>";
	...
	echo "<TD colspan=3 align=center height=1></TD>";
	echo "</TR><TR >";
	echo "<TD class=\"col-md-3\" >$label_email: &nbsp; </TD><TD class=\"col-md-3\" ALIGN=left><div class=\"col-lg-12\"><input class=\"form-control\" type=text name=email size=15 maxlength=30></div></TD>";
	echo "<TD class=\"col-md-5\" ><INPUT class='btn btn-bdr-sucess' TYPE=SUBMIT NAME=SUBMIT VALUE='"._QXZ("SUBMIT")."'></TD>\n";
	echo "</TR><TR>";
	echo "</TR>";
	...
	if ( (strlen($log_lead_id_archive)>0) or (strlen($log_phone_archive)>0) )
		...
		if ( ($results_to_print < 1) and ($results_to_printX < 1) )
			...
			{
			echo "<div class=\"col-md-12\"><div class=\"content\">";	
			echo "<center><BR><b>"._QXZ("INBOUND LOG RESULTS").": $results_to_print</b></center>\n";
			echo "<TABLE BGCOLOR=WHITE CELLPADDING=1 CELLSPACING=0 WIDTH=770>\n";
			echo "<TR >\n";
			echo "<TD ALIGN=LEFT VALIGN=TOP><B>#</B></FONT></TD>\n";
			echo "<TD ALIGN=CENTER VALIGN=TOP><B>"._QXZ("LEAD ID")."</B> &nbsp;</FONT></TD>\n";
	...
	if ( ($results_to_print < 1) and ($results_to_printX < 1) )
		...
		while ($results_to_print > $o)
			...
			echo "<TD ALIGN=CENTER><a href=\"admin_modify_lead.php?lead_id=$row[0]&archive_search=$archive_search&archive_log=$log_archive_link\" target=\"_blank\">$row[0]</a></FONT></TD>\n";
