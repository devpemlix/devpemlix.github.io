...
if ( (!$OK_to_process) or ( ($leadfile) and ($file_layout!="standard" && $file_layout!="template") ) )
	...
	if ($file_layout!="custom") 
		...
		?>
			  ...
			  <div class="col-md-12">
			  <div class="content">
		<table class="col_table table1" align=center border=0 cellpadding=5 cellspacing=0 >
		  <tr>
		  ...
		  <tr>
			...
			while ( $num_rows > $count ) 
				{
				$row = mysqli_fetch_row($rslt);
				echo "<option value=\'$row[0]\'>$row[0] - $row[0]</option>\n";
				$count++;
				}
			?>
			</select></div><div class="col-lg-5"><input class="ace"  type='checkbox' name='master_list_override' value='1'><label class="lbl"></label>(override template setting)
			</font></td>
		  </tr>
		  <tr>
		  ...
		  <tr>
			<td><B><font face="arial, helvetica" size=2>Custom template to use:</font></B></td>
...
?>
		  ...
		  <tr>
			...
			<option value="DUPDELE"><?php echo _QXZ("DELETE PHONE NUMBER BY PHONE IN LIST ID"); ?></option>
			<!--<option value="DUPCAMP"><?php echo _QXZ("CHECK FOR DUPLICATES BY PHONE IN ALL CAMPAIGN LISTS"); ?></option>-->
			<!--<option value="DUPSYS"><?php echo _QXZ("DELETE FOR DUPLICATES BY PHONE IN ENTIRE SYSTEM"); ?></option>-->
	...
	<tr >
		...
		<tr>
			<td></td>
			<td><div class='col-lg-5'><input class="btn btn-bdr-sucess" type=submit value="<?php echo _QXZ("SUBMIT"); ?>" name='submit_file'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input class="btn btn-bdr-pink" type=button onClick="javascript:document.location='admin_listloader_fourth_gen.php'" value="<?php echo _QXZ("START OVER"); ?>" name='reload_page'></div></td>
		  </tr>
		  <tr><td align=left><font size=1> &nbsp; &nbsp; &nbsp; &nbsp; <a href="admin.php?ADD=100" target="_parent"><?php echo _QXZ("BACK TO ADMIN"); ?></a> &nbsp; &nbsp; &nbsp; &nbsp; <a href="./admin_listloader_third_gen.php"><?php echo _QXZ("Old Lead Loader"); ?></a> &nbsp; &nbsp; </font></td><td align=right><font size=1><?php echo _QXZ("LIST LOADER 4th Gen"); ?>  </td></tr>
	...
	<input type=hidden name='leadfile_name' value="<?php echo $leadfile_name ?>">
	<input type=hidden name='DB' value="<?php echo $DB ?>">
	<a href="admin_listloader_fourth_gen.php"><?php echo _QXZ("Load Another Lead File"); ?></a> &nbsp; &nbsp; &nbsp; &nbsp;</font></B> 
	</font></td>
	</tr></table>
		...
		while (!feof($file)) 
			...
			if (strlen($buffer)>0) 
				...
				if (preg_match("/DUPSYS/i",$dupcheck))
					{
					$dup_lead=0;
					$stmt="delete from vicidial_list where phone_number='$phone_number' $statuses_clause;";
					$rslt=mysql_to_mysqli($stmt, $link);
					$pc_recs = mysqli_num_rows($rslt);
					if ($pc_recs > 0)
						{
				...
				if (preg_match("/DUPLIST/i",$dupcheck))
					...
					##### Check for delete phone numbers in vicidial_list table for one list_id #####
				if (preg_match("/DUPDELE/i",$dupcheck))
					{
					$dup_lead=0;
					$stmt="delete from vicidial_list where phone_number='$phone_number'and list_id='$list_id'$statuses_clause;";
					$rslt=mysql_to_mysqli($stmt, $link);
					$pc_recs = mysqli_num_rows($rslt);
					if ($pc_recs > 0)
						{
