	...
	if ($modify_languages==1)
		...
		function edit_translation(TEXTspan,LINKspan)
			...
			if (active_edit > 0)
				...
				document.getElementById(TEXTspan).innerHTML = "<div class='col-lg-5'><TEXTAREA class='form-control' name=\"active_translate\" id=\"active_translate\" ROWS=3 COLS=70>" + translated_text_value + "</TEXTAREA></div><input type=hidden name=\"original_translate\" id=\"original_translate\" value=\"" + translated_text_value + "\">";
				}
			}
		...
		if ($action=='NOTTRANSLATED')
			...
			$displaySQL="and ( (translated_text='') or (translated_text is NULL) )";
				echo "<tr><td class=size' align='center' colspan='5' style='background:#fff;'>"._QXZ("ONLY NOT TRANSLATED PHRASES")."</td></tr><BR>\n";
			}
		elseif ($action=='TRANSLATED')
			{
			$displaySQL="and ( (translated_text!='') or (translated_text is not NULL) )";
			echo "<tr><td class=size' align='center' colspan='5' style='background:#fff;'>"._QXZ("ONLY TRANSLATED PHRASES")."</td></tr><BR>\n";
			}
		else
			{
				echo "<tr><td class=size' align='center' colspan='5' style='background:#fff;'>"._QXZ("ALL PHRASES")."</td></tr><BR>\n";
			}
		...
		if ($language_exists > 0)
			...
			while ($phrases_to_print > $o) 
				...
				if (strlen($translated_text) < 1)
					{$ttbgcolor='';}
				echo "<tr >";
				echo "<td align=left width=100><B>$o</B></td>";
				echo "<td align=left width=100>"._QXZ("ID").": $phrase_id</td>";
				echo "<td align=left width=100>"._QXZ("user").": $source</td>";
				echo "<td align=left width=300>"._QXZ("last modified").": $modify_date</td>";
				echo "<td align=right width=200><a href=\"$PHP_SELF?ADD=463111111111&language_id=$language_id&DB=$DB&action=$action&phrase_id=$phrase_id&stage=PHRASEDELETE\" target=\"_parent\">"._QXZ("delete phrase")."</a></td></tr>\n";
...
if ($ADD==163111111111)
	...
	if ($modify_languages==1)
		...
		echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"header\">
					<h3>"._QXZ("ADD NEW LANGUAGE ENTRY")."</h3>
					</div>";
		...
		echo "<tr ><td align=right nowrap>"._QXZ("Language Description").": </td><td align=left nowrap><div class='col-lg-5'><input class='form-control' type=text name=language_description size=70 maxlength=255></div><!--$NWB#languages-language_description$NWE--></td></tr>\n";
		...
		if ($gathered_phrases_TOTAL > 0)
			{
			echo "<tr ><td align=right>"._QXZ("Populate Language").": </td><td align=left><div class='col-lg-5'><select class='form-control' size=1 name=stage><option SELECTED value='NONE'>"._QXZ("Leave Empty")."</option><option value='ALL'>"._QXZ("All Gathered Phrases").": $gathered_phrases_TOTAL</option><option value='AGENT'>"._QXZ("Agent Phrases Only").": $gathered_phrases_AGC</option><option value='ADMIN'>"._QXZ("Admin Phrases Only").": $gathered_phrases_ADMIN</option></select></div><!--$NWB#languages-populate$NWE--></td></tr>\n";
			}
		else
			{
			echo "<tr ><td align=right><input type=hidden name=stage value=\"NONE\"></td></tr>\n";
			}
...
if ($ADD==163211111111)
	...
	if ($modify_languages==1)
		...
		echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"block-flat\">
					<div class=\"header\">
					<h3>"._QXZ("COPY A LANGUAGE ENTRY")."</h3>
					</div>";
		echo "<form action=$PHP_SELF method=POST>\n";
...
if ($ADD==163311111111)
	...
	if ($modify_languages==1)
		...
		echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"block-flat\">
					<div class=\"header\">
					<h3>"._QXZ("IMPORT LANGUAGE PHRASES")."</h3>
					</div>";
		echo "<form action=$PHP_SELF method=POST>\n";
		...
		echo "<tr ><td align=right width=30%>"._QXZ("Language ID").": </td><td align=left><div class='col-lg-5'><select class='form-control' size=1 name=language_id>\n";
		...
		echo "<tr><td align=right>"._QXZ("Import Data").": </td><td align=left><div class='col-lg-5'><TEXTAREA class='form-control' name=\"import_data\" id=\"import_data\" ROWS=40 COLS=120></TEXTAREA></div><!--$NWB#languages-import_data$NWE--></td></tr>\n";
...
if ($ADD==163411111111)
	...
	if ($modify_languages==1)
		...
		echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"block-flat\">
					<div class=\"header\">
					<h3>"._QXZ("EXPORT LANGUAGE PHRASES")."</h3>
					</div>";
		echo "<form action=$PHP_SELF method=POST>\n";
...
if ($ADD==263111111111)
	...
	if ($add_copy_disabled > 0)
		...
		echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
		echo "&nbsp;&nbsp;"._QXZ("You do not have permission to add records on this system")." -system_settings-\n";
		...
		if ($row[0] > 0)
			{
				echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
				echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY NOT ADDED - there is already a language entry in the system with this ID")."\n";
				echo "</div></div></div></div></div>";
			}
		else
			{
			if ( (strlen($language_id) < 2) or (strlen($language_description) < 3) )
				{
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
					echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY NOT ADDED - Please go back and look at the data you entered")."\n";
					echo "</div></div></div></div></div>";
				}
			else
				{
				echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
				echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY ADDED")."\n";
				echo "</div></div></div></div></div>";
				...
				if (preg_match("/ALL|AGENT|ADMIN/",$stage))
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
					echo "&nbsp;&nbsp;"._QXZ("POPULATED WITH GATHERED PHRASES").": $affected_rowsA\n";
					echo "</div></div></div></div></div>";
					}
		...
		if ($row[0] > 0)
			...
			if ( (strlen($language_id) < 2) or (strlen($language_description) < 3) or (strlen($source_language_id) < 2) )
				...
				if ($row[0] < 1)
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
						echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY NOT COPIED - invalid source language")."\n";
						echo "</div></div></div></div></div>";
					}
				else
					{
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
					echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY COPIED")."\n";
					echo "</div></div></div></div></div>";
	...
	if ($row[0] < 1)
		{
			echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
			echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY DOES NOT EXIST")."\n";
			echo "</div></div></div></div></div>";
		}
	else
		{
		if ( (strlen($import_data) < 2) or (strlen($stage) < 1) )
			{
				echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
				echo "&nbsp;&nbsp;"._QXZ("LANGUAGE IMPORT NOT PROCESSED - Please go back and look at the data you entered")."\n";
				echo "</div></div></div></div></div>";
			}
		else
			{
			...
			if ( (strlen($language_id) < 2) or (strlen($phrase_id) < 1) )
				{
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
					echo "&nbsp;&nbsp;"._QXZ("LANGUAGE PHRASE NOT MODIFIED - Please go back and look at the data you entered")."$language_id|$phrase_id\n";
					echo "</div></div></div></div></div>";
				}
			else
				{
				...
				echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
				echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY MODIFIED").": $language_id - $audio_filename\n";
				echo "</div></div></div></div></div>";
			...
			if ( (strlen($language_id) < 2) or (strlen($language_description) < 3) )
				{
					...
					<div class=\"row\">
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\" style='margin-bottom:15px;'>";
					echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY NOT MODIFIED - Please go back and look at the data you entered")."\n";
					echo "</div></div></div></div></div>";
				}
			else
				{
					...
					if ($phrase_exists > 0)
						{
							echo "
								...
								<div class=\"col-md-12 no-padding\">
								<div class=\"block-flat\" style='margin-bottom:15px;'>";
							echo "&nbsp;&nbsp;"._QXZ("Phrase already exists in this language ID").": $language_id, $english_text\n";
						...
						echo "<div id=\"pcont\" class=\"container-fluid\">
								...
								<div class=\"col-md-12 no-padding\">
								<div class=\"block-flat\" style='margin-bottom:15px;'>";
						echo "&nbsp;&nbsp;"._QXZ("New phrase added to this language ID").": $language_id, $english_text\n";
						echo "</div></div></div></div></div>";
				...
				echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\" style='margin-bottom:15px;'>";
				echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY MODIFIED").": $language_id\n";
				echo "</div></div></div></div></div>";
...
if ($ADD==563111111111)
	...
	if (strlen($language_id) < 2)
		...
		echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";	
		echo "&nbsp;&nbsp;"._QXZ("LANGUAGE ENTRY NOT DELETED - Please go back and look at the data you entered")."\n";
		...
		echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
		echo "&nbsp;&nbsp;<B>"._QXZ("LANGUAGE ENTRY DELETION CONFIRMATION").": $language_id - $language_description</B>\n";
...
if ($ADD==663111111111)
	...
	if ( (strlen($language_id) < 2) or ($ConFiRm != 'YES') )
		...
		echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"col-md-12 no-padding\">
					<div class=\"block-flat\">";
		echo "&nbsp;&nbsp;<B>"._QXZ("LANGUAGE DELETION COMPLETED").": $language_id</B>\n";
		echo "</div></div></div></div></div>";
		echo "<br><br>\n";
		}
...
if ($ADD==363111111111)
	...
	if ($modify_languages==1)
		...
		echo "<div id=\"pcont\" class=\"container-fluid\">
					...
					<div class=\"block-flat\">
					<div class=\"header\">
					<h3>"._QXZ("MODIFY A LANGUAGE RECORD").": $language_id</h3>
					</div>";
		...
		echo "<center><div class=\"col-md-12\"><div class=\"content\"><TABLE class='hover' width=$section_width cellspacing=3>\n";
		echo "<tr><td align=right>"._QXZ("Language ID").": </td><td align=left><div class='col-lg-5'><B>$language_id</B></div></td></tr>\n";
		echo "<tr><td align=right>"._QXZ("Last Modified").": </td><td align=left><div class='col-lg-5'>$modify_date</div></td></tr>\n";
		echo "<tr><td align=right nowrap>"._QXZ("Language Description").": </td><td align=left nowrap><div class='col-lg-5'><input class='form-control' type=text name=language_description size=70 maxlength=255 value=\"$language_description\"></div><!--$NWB#languages-language_description$NWE--></td></tr>\n";
		...
		echo "<iframe src=\"admin_languages.php?ADD=363211111111&language_id=$language_id&DB=$DB&action=$action\" name=\"language_phrases\" style=\"background-color:transparent;\" scrolling=\"auto\" frameborder=\"0\" allowtransparency=\"true\" width=\"100%\" height=\"400\">\n</iframe>\n";
		...
		echo "<tr><td align=center valign=middle colspan=2><input class='btn btn-bdr-sucess' type=submit name=submit VALUE='"._QXZ("SUBMIT CHANGES")."'> &nbsp; <a href=\"$PHP_SELF?ADD=363111111111&language_id=$language_id&DB=$DB&action=$action\">RELOAD PAGE</a></td></tr>\n";
...
if ($ADD==163000000000)
	...
	echo "<center><div class=\"col-md-12\"><div class=\"content\"><TABLE class='hover' width=$section_width cellspacing=0 cellpadding=1>\n";
	echo "<tr >";
	echo "<td><B>"._QXZ("LANGUAGE ID")."</B></td>";
	echo "<td><B>"._QXZ("Description")."</B></td>";
	echo "<td><B>"._QXZ("Code")."</B></td>";
	echo "<td><B>"._QXZ("ACT")."</B></td>";
	echo "<td><B>"._QXZ("ADMIN GROUP")."</B></td>";
	echo "<td><B>"._QXZ("MODIFY DATE")."</B></td>";
	echo "<td><B>"._QXZ("PHRASES")."</B></td>";
	echo "<td align=center><B>"._QXZ("MODIFY")."</B></td></tr>\n";
	...
	while ($lang_to_print > $o) 
		...
		if (preg_match('/1$|3$|5$|7$|9$/i', $o))
			...
			{$bgcolor='';}
		echo "<tr $bgcolor><td><a href=\"$PHP_SELF?ADD=363111111111&language_id=$Alanguage_id[$o]&DB=$DB\">$Alanguage_id[$o]</a></td>";
		...
		echo "<td align=center><a  href=\"$PHP_SELF?ADD=363111111111&language_id=$Alanguage_id[$o]&DB=$DB\">"._QXZ("MODIFY")."</a></td></tr>\n";
		$o++;
		}
...
if ($ADD == "763000000000")
	...
	if ($LOGuser_level >= 9)
		...
		echo "<TD><B>"._QXZ("IP")."</B></TD>\n";
		echo "<TD><B>"._QXZ("SECTION")."</B></TD>\n";
		echo "<TD><B>"._QXZ("TYPE")."</B></TD>\n";
		echo "<TD><B>"._QXZ("RECORD ID")."</B></TD>\n";
		echo "<TD><B>"._QXZ("DESCRIPTION")."</B></TD>\n";
		echo "<TD><B>"._QXZ("GOTO")."</B></TD>\n";
		echo "</TR>\n";
		...
		while ($logs_to_print > $o)
			...
			if (preg_match('/1$|3$|5$|7$|9$/i', $o))
				...
				{$bgcolor='';}
			echo "<tr $bgcolor><td><a href=\"admin.php?ADD=730000000000000&stage=$row[0]&DB=$DB\">$row[0]</a></td>";
			echo "<td> $row[1]</td>";
