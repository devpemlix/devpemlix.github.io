...
if ($NEW_VOICE_LAB > 0)
	...
	if ($PLAY_MESSAGE > 0)
		...
		{
		echo "<table style='width:50%'><tr colspan=2 class='border-bottom'><td>";
		echo "<b>"._QXZ("Start a Voice Lab Session").": 8600900</b></td></tr><tr><td><form action=$PHP_SELF method=POST>\n";
		echo "<input type=hidden name=NEW_VOICE_LAB value=2>\n";
		echo "<input type=hidden name=session_id value=8600900>\n";
		echo _QXZ("Your Server").":</td><td> <select class='form-control' size=1 name=server_ip>$servers_list</select></td></tr><tr><td>\n";
		echo "\n";
		echo _QXZ("Campaign").": </td><td><select  class='form-control'size=1 name=campaign_id>$campaigns_list</select></td></tr>";
		echo "\n";
		echo "<td colspan=2 align=center><input type=submit name=submit value='"._QXZ("submit")."' class='btn btn-bdr-sucess'></td></tr>\n";
		echo "</form>\n";

		echo "<table style='width:50%'><tr colspan=2 class='border-bottom'><td>";
		echo "<b>"._QXZ("Kill a Voice Lab Session").": 8600900</b></td></tr><tr><td><form action=$PHP_SELF method=POST>\n";
		echo "<input type=hidden name=KILL_VOICE_LAB value=2>\n";
		echo "<input type=hidden name=session_id value=8600900>\n";
		echo _QXZ("Your Server").": </td><td><select class='form-control' size=1 name=server_ip>$servers_list</select></td></tr><tr><td>\n";
		echo "\n";
...
if ($KILL_VOICE_LAB > 1)
	...
	if ( (strlen($server_ip) > 6) && (strlen($session_id) > 6) && (strlen($campaign_id) > 2) )
		...
		echo "<pre style='width:1000px;float:left'>|$stmt|</pre><br/><br/><br/><br/>\n";
		$rslt=mysql_to_mysqli($stmt, $link);
