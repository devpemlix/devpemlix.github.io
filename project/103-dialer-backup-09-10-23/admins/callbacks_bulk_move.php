...
<div id="pcont" class="container-fluid">
		<div class="cl-mcont">
        <div class="row">
		<div class="col-md-12 no-padding"><div class="block-flat"><div class="header"> <h3> <?php echo _QXZ("Callbacks Bulk Move"); ?></h3> </div>
...
#		}
	...
	echo "<table style='width:50%' align='center'><tr><td align='left'>";
	if ($purge_called_records) 
		{
	...
	echo "<center><a class='btn btn-bdr-sucess' href='$PHP_SELF?DB=$DB&new_list_id=$new_list_id&new_status=$new_status&days_uncalled=$days_uncalled&purge_called_records=$purge_called_records&SUBMIT=1&confirm_transfer=YES$usersQS$user_groupsQS$listsQS$groupsQS'>"._QXZ("CLICK TO CONFIRM")."</a><BR><BR>";
	echo "<a class='btn btn-bdr-sucess' href='$PHP_SELF?DB=$DB'>"._QXZ("CLICK TO CANCEL")."</a></center><BR><BR>";
	echo "</td></tr></table>";
	}
else 
	{
	# JOEJ - removed "recipient='USERONLY' and status!='INACTIVE'" from all select statements in this section 2/17/15
	echo "<table style='width:70%' align='center' border=0>";
	echo "<tr>";
	echo "<td align='left' width='270' style='padding:15px;'>";
	...
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {echo "$stmt\n";}
	echo "<B style='color:#58595b;'>"._QXZ("Campaigns with callbacks").":</B><BR><select name='cb_groups[]' size=5 multiple class='form-control'>\n";
	...
	echo "<td align='left' width='400' style='padding:15px;'>";
	...
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {echo "$stmt\n";}
	echo "<B style='color:#58595b'>"._QXZ("Lists with callbacks").":</B><BR><select name='cb_lists[]' size=5 multiple class='form-control'>\n";
	...
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {echo "$stmt\n";}
	echo "<B style='color:#58595b'>"._QXZ("User groups with callbacks").":</B><BR><select name='cb_user_groups[]' size=5 multiple class='form-control'>\n";
	...
	$rslt=mysql_to_mysqli($stmt, $link);
	if ($DB) {echo "$stmt\n";}
	echo "<B style='color:#58595b'>"._QXZ("Agents with callbacks").":</B><BR><select name='cb_users[]' size=5 multiple class='form-control'>\n";
	...
	echo "<B><input type='checkbox' name='purge_called_records' value='Y' checked class='ace'><label class='lbl'>"._QXZ("Purge called records").'</label>';
	echo "</td>";
	echo "<td align='left' width='400' style='padding:15px;'>";
	echo "<div class='col-lg-5 no-padding' style='color:#58595b;margin-top:8px'><b>"._QXZ("Live and uncalled for over")."</b></div><div class='col-lg-2 no-padding'><select name='days_uncalled' class='form-control'>";
	...
	echo "</select></div>";
	echo "<div class='col-lg-5 no-padding' style='color:#58595b;margin-top:8px;'>&nbsp;<b>"._QXZ("days,leave blank to ignore")."</b></div></div>";
	echo "</td>";
	echo "</tr>";

	echo "</table>";

	echo "<BR/><BR/><input type='submit' name='SUBMIT' value='  "._QXZ("TRANSFER TO")." ' class='btn btn-bdr-sucess'><BR/><BR/>";

	$stmt="SELECT list_id, list_name from vicidial_lists $whereLOGadmin_viewable_groupsSQL order by list_id asc";
	if ($DB) {echo "$stmt\n";}
	$rslt=mysql_to_mysqli($stmt, $link);
	echo "<table style='width:70%;' ><tr><td style='color:#58595b;'><b>";
	echo _QXZ("List ID").":</b></td><td><select name='new_list_id' size=1 class='form-control'>\n";
	echo "<option value='X'>"._QXZ("DO NOT ALTER LIST ID")."</option>";
	while ($row=mysqli_fetch_array($rslt)) 
		{
		echo "\t<option value='$row[list_id]'>$row[list_id] - $row[list_name]</option>\n";
		}
	echo "</select> </td><td style='color:#58595b;'><b>\n";
	echo _QXZ("New Status").":</b></td><td><input type=text name='new_status' size=7 maxlength=6 value='$new_status' class='form-control'></td></td></table>\n";
	}
echo "</form>\n";
