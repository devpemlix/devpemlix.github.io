...
<CENTER>
<TABLE style='width:30%;'  cellpadding=2 cellspacing=0>
...
if ($SUBMIT && $old_user && $new_user && !$confirm_transfer) 
	...
	echo "<a href='$PHP_SELF?DB=$DB&old_user=$old_user&new_user=$new_user&confirm_transfer=1&SUBMIT=1' class='btn btn-bdr-sucess'>"._QXZ("CLICK TO CONFIRM")."</a><BR><BR>";
	echo "<a href='$PHP_SELF?DB=$DB' class='btn btn-bdr-sucess'>"._QXZ("CLICK TO CANCEL")."</a><BR><BR>";
	} 
else 
	{
	...
	if ($DB) {echo "$stmt\n";}
	# <tr><td align='right'>
	echo ""._QXZ("Agents with callbacks").":<BR><select name='old_user' size=5  class='form-control'>\n";
	...
	if ($DB) {echo "$stmt\n";}
	$rslt=mysql_to_mysqli($stmt, $link);
	echo "<select name='new_user' size=5 class='form-control'>\n";
	while ($row=mysqli_fetch_array($rslt)) 
		{
